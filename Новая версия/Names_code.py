...
def regex(morph):
    ...
    specials = ['Рыков', 'Кампрад', 'Рудзутак', 'Брэнсон', 'Брин', 'Каменев', 'Теллер', 'Фридан', 'Горбачёв', 'Найт', 'Дьюар', 'Бик', 'Клаузевиц', 'Бах', 'Булгаиин', 'Белл', 'Делл', 'Баффетт', 'Лютер', 'Зворыкин', 'Бентам', 'Диор', 'Эттингер']
    line = ''
    for name in names:
        if ' ' in name: # - проверяю, если слово состоит из двух частей
            ...
            if name in specials:
                ...
                name1 = name[:-1] + '(?:ь|я|е|ю|ем|ях|ям|ями|ях|ей|и)' + '\\b' + '|'
                line = line + name1
            else:
                frm = form(morph, name)
